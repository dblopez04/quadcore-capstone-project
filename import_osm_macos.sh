...
if [ -n "${OSMIUM_IMAGE:-}" ]; then
    ...
    OSMIUM_IMAGES=("iboates/osmium:latest")
fi
...
is_valid_osm_xml() {
    local path="$1"
    [ -f "$path" ] && file "$path" | grep -q "OpenStreetMap XML data"
}
...
extract_map_with_osmium() {
    ...
    for image in "${OSMIUM_IMAGES[@]}"; do
        ...
        if docker run --rm -v "$(pwd)/$OSM_DIR":/data "$image" \
            extract -b "$DENTON_COBOX" /data/texas-latest.osm.pbf -o /data/map.osm --overwrite; then
            return 0
        fi
...
if [ "$FORCE_MAP_REFRESH" = "1" ] || ! is_valid_osm_xml "$OSM_FILE"; then
