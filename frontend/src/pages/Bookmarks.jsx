...
export default function Bookmarks() {
    ...
    const rows = useMemo(() => {
        const mapped = bookmarks.map((bookmark, index) => {
            ...
            return {
                ...
                shareUrl: location?.share_url || null,
                ...coordinates,
            };
        });
    ...
    useEffect(() => {
        ...
        loadBookmarkData();
        // `loadBookmarkData` is intentionally kept local so handlers can await it directly.
    ...
    useEffect(() => {
        ...
        function onClickOutside(event) {
            if (!event.target.closest(".bk-card-menu")) {
                setOpenMenuId(null);
            }
        }
    ...
    function handleCardClick(row) {
        if (row.lat == null && !row.shareUrl) return;
        handleOpen(row);
    }
                        ...
                        {!loading && !error && rows.length === 0 && (
                            <div className="bk-empty">
                                ...
                                <p className="bk-empty-hint">
                                    {selectedListId === ALL_BOOKMARKS_LIST_ID
                                        ? "Save locations from the search or map to see them here."
                            ...
                            <div className="bk-cards">
                                {rows.map((row, index) => {
                                    ...
                                    return (
                                        <div
                                            key={row.rowId}
                                            className={`bk-card${hasCoords || row.shareUrl ? " bk-card--clickable" : ""}${menuOpen ? " bk-card--menu-open" : ""}`}
